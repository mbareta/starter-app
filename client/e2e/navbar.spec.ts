...
test('Check navbar item elements', async ({page}) => {
    ...
    await test.step('Verify the user avatar and email display', async () => {
        await page.goto('/')
        await expect(page.locator(`//img[@alt="user's profile picture"]`)).toBeVisible();
        await expect(page.locator(`//div[@class="navbar-item has-dropdown is-hoverable"]//div[contains(text(),"${process.env.ADMIN_EMAIL}")]`)).toBeVisible();
    }),
